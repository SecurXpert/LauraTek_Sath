...
const ConsoleOutput: React.FC<ConsoleOutputProps> = ({ output, testResults }) => {
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 flex-shrink-0">
      <div className="bg-[#EAE4FF] rounded-[24px] p-4 shadow-sm flex flex-col h-full min-h-[160px] md:col-span-2">
        ...
        <div className="bg-[#0C0A15] rounded-[16px] p-4 font-mono text-[11px] text-gray-400 flex-1 leading-relaxed shadow-inner overflow-y-auto max-h-[200px] whitespace-pre-wrap">
          {output ? (
      ...
      <div className="bg-[#F5F3FF] rounded-[24px] p-5 shadow-sm flex flex-col h-full min-h-[160px]">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-full bg-[#10B981] flex items-center justify-center text-white shadow-sm"><CheckCircle className="w-3 h-3" /></div>
            <span className="text-[14px] font-bold text-slate-800">Status</span>
          </div>
          {testResults ? (
            <span className="text-[10px] font-bold text-[#10B981] bg-white px-3 py-1 rounded-full shadow-[0px_2px_4px_rgba(0,0,0,0.02)]">Evaluated</span>
          ) : (
            <span className="text-[10px] font-bold text-gray-400 bg-white px-3 py-1 rounded-full shadow-[0px_2px_4px_rgba(0,0,0,0.02)]">Pending</span>
          )}
        </div>
        <div className="flex-1 flex flex-col justify-center px-1">
          ...
          <div className="w-full h-1.5 bg-[#E6F9F0] rounded-full">
            <div 
              className="h-full bg-[#10B981] rounded-full shadow-[0px_0px_8px_rgba(16,185,129,0.4)] transition-all"
