...
const QuizAlert: React.FC<QuizAlertProps> = ({ onEnrollClick }) => {
  return (
      <div 
        className="rounded-[16px] p-4 lg:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 shadow-guest"
        style={{ background: '#FEF3C7', borderTop: '1.22px solid #FDE68A' }}
      >
        ...
        <button 
          ...
          className="whitespace-nowrap px-8 py-2.5 bg-[#5B4FFF] hover:bg-[#4a3fdb] text-white text-[13px] font-bold rounded-full shadow-guest transition-colors self-start sm:self-auto"
        >
          Enroll
        </button>
      </div>
