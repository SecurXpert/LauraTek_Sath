...
interface DashboardCourseCardProps {
  course: any;
  setSelectedCurriculumCourse: (course: any) => void;
}

const DashboardCourseCard: React.FC<DashboardCourseCardProps> = ({ course, setSelectedCurriculumCourse }) => {
  ...
  const isThird = course.title.includes("UI/UX") || course.status === "completed" || course.id === 3;

  const totalMods = isFirst ? 10 : isSecond ? 8 : isThird ? 12 : 10;
  const completedMods = isThird ? totalMods : isFirst ? 6 : isSecond ? 3 : Math.round((course.progress / 100) * totalMods);

  const weeksStr = `${course.duration || 0} hrs`;
  const ratingStr = (course.rating ?? 0).toFixed(1);
  ...
  const authorImg = course.authorImg || fallbackAvatar;
    ...
    >
      ...
      <div className="p-5 sm:p-6 md:p-7 flex flex-col flex-1">
        ...
        <div className="flex items-center gap-3.5 mb-6 pb-6 border-b border-gray-100/80">
            {course.authorImg && course.authorImg.trim() !== "" ? (
          ...
          <div>
            ...
            <p className="text-[12px] text-gray-500 font-medium leading-none">Instructor</p>
          </div>
        </div>
        ...
        <div className="flex flex-col mt-auto">
          <div className="mb-6">
            <div className="flex justify-between items-center mb-2">
              <span className="text-[13px] font-bold text-gray-900">{course.progress}% Complete</span>
              <span className="text-[13px] font-medium text-gray-500">{course.completed_modules ?? completedMods}/{course.total_modules ?? totalMods} Modules</span>
            </div>
          ...
          <div className="flex items-center justify-between mb-6 px-1 w-full">
            ...
            <span className="flex items-center gap-1.5 text-[13.5px] font-bold text-gray-900">
              <Star size={16} className="text-[#FFC200] fill-[#FFC200]" /> {ratingStr}
            </span>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-2.5 w-full pt-2">
            <button
              ...
              onClick={() => navigate(`/course1/${course.id}`, { state: { course, from: 'dashboard' } })}
            >
            ...
            <button
              ...
              onClick={() => setSelectedCurriculumCourse(course)}
            >
              Curriculum
            </button>
            <button
              className="flex-[1_1_calc(50%-6px)] xl:flex-[1_1_0%] min-w-[100px] bg-[#F9FAFB] text-gray-600 text-[12.5px] sm:text-[13px] font-semibold py-3 sm:py-3.5 px-2 sm:px-3 rounded-[16px] border border-gray-200 hover:bg-gray-100 transition-colors text-center whitespace-nowrap shadow-sm"
              onClick={() => navigate(`/materials/${course.id}`, { state: { from: 'dashboard', courseTitle: course.title } })}
            >
              Materials
            </button>
          </div>
