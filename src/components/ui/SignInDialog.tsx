...
//   // Unified Login – Works for Guest & Student
...
//       // Guest User → Direct access_token
...
//     e.preventDefault();
//     if (!tempToken || (!otp && !backupCode.trim())) return;
...
//           code: showBackupInput ? backupCode.trim() : otp,
//         }),
//       });
...
//         if (showBackupInput && data.mfa_re_enroll_required !== false) {
//           await startReEnroll();
//         } else {
...
//   // Re-enroll after backup code
//   const startReEnroll = async () => {
//     if (!tempToken) return;
//     setLoading(true);
//     try {
//       await fetch(`${BASE_URL}/mfa/re-enroll/start`, {
//         method: "POST",
//         headers: { "Content-Type": "application/json" },
//         body: JSON.stringify({ token: tempToken }),
//       });
...
//       setQrImage(URL.createObjectURL(blob));
//       setStep("reenroll");
//       setError("Scan with your new device");
//     } catch {
...
//   // Verify Enrollment or Re-enroll
//   const handleVerifySetup = async (e: React.FormEvent, isReEnroll = false) => {
//     e.preventDefault();
...
//         method: "POST",
//         headers: { "Content-Type": "application/json" },
//         body: JSON.stringify({ token: tempToken, data: { code: otp } }),
//       });
...
//                     type="submit"
//                     disabled={loading}
//                     className="w-full bg-gradient-to-r from-indigo-600 to-purple-700 hover:from-indigo-700 hover:to-purple-800 text-white font-bold py-7 text-xl rounded-full shadow-xl transform transition hover:scale-105"
//                   >
...
//                   <Button
//                     type="submit"
//                     disabled={loading || (!showBackupInput && otp.length !== 6) || (showBackupInput && !backupCode.trim())}
//                     className="w-full bg-gradient-to-r from-indigo-600 to-purple-700 py-7 text-xl font-bold rounded-full"
//                   >
//                     Verify & Continue
//                   </Button>
//                 </form>
...
//               {/* Enrollment */}
//               {step === "enroll" && (
//                 <form onSubmit={(e) => handleVerifySetup(e, false)} className="space-y-8 text-center">
...
//                   {qrImage ? (
//                     <img src={qrImage} alt="QR Code" className="w-64 h-64 mx-auto border-4 border-gray-300 rounded-2xl shadow-lg" />
//                   ) : (
...
//               {/* Re-enroll */}
//               {step === "reenroll" && (
//                 <form onSubmit={(e) => handleVerifySetup(e, true)} className="space-y-8 text-center">
//                   <RefreshCw className="w-16 h-16 text-orange-600 mx-auto" />
//                   <h3 className="text-2xl font-bold">Restore Access</h3>
//                   <p className="text-gray-600">Scan with your new device</p>
//                   {qrImage && <img src={qrImage} alt="Recovery QR" className="w-64 h-64 mx-auto border-4 border-gray-300 rounded-2xl shadow-lg" />}
//                   <Input
//                     value={otp}
//                     onChange={(e) => setOtp(e.target.value.replace(/\D/g, "").slice(0, 6))}
...
//                   />
//                   {error && <p className="text-red-600">{error}</p>}
//                   <Button type="submit" disabled={loading || otp.length !== 6} className="w-full bg-gradient-to-r from-orange-600 to-red-700 py-7 text-xl font-bold rounded-full">
//                     Finish Recovery
//                   </Button>
//                 </form>
//               )}
...
//                   <p className="text-gray-600">Save these backup codes safely</p>
