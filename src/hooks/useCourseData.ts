...
export const useCourseData = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const materialsRes = await fetch(
          `${import.meta.env.VITE_API_URL}/courses/${courseData?.course_id || 1}/materials`,
          {
            method: "GET",
            headers: { Authorization: `Bearer ${token}` },
          }
        ...
        const curriculumRes = await fetch(
          `${import.meta.env.VITE_API_URL}/courses/${courseData?.course_id || 1}/curriculum`,
          {
            method: "GET",
            headers: { Authorization: `Bearer ${token}` },
          }
        ...
        const certificatesRes = await fetch(
          `${import.meta.env.VITE_API_URL}/student/student/my-certificates`,
          {
            method: "GET",
            headers: { Authorization: `Bearer ${token}` },
          }
        ...
        if (certificatesRes.ok) {
          ...
          if (studentId && courseData?.course_id) {
            const progressRes = await fetch(
              `${import.meta.env.VITE_API_URL}/courses/students/${studentId}/courses/${courseData.course_id}/progress`,
              {
                method: "GET",
                headers: { Authorization: `Bearer ${token}` },
              }
  ...
  const handleDownload = async (materialId: number, title: string) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/courses/${course?.course_id || 1}/materials/${materialId}/download`, {
        headers: { Authorization: `Bearer ${token}` }
      });
