...
import { Question, Result } from "@/components/ui/quiz-page/types";
...
export function useQuizData(id: string | undefined) {
  ...
  const fetchQuestions = useCallback(async () => {
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/dashboard/quiz-view/${id}`,
        {
          headers: {
  ...
  const handleSubmit = useCallback(async () => {
    try {
      ...
      await fetch(`${import.meta.env.VITE_API_URL}/quiz/submit`, {
        method: "POST",
        headers: {
